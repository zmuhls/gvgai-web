...
5. **History** (`historyContext`) — last 3 actions with score/health/position deltas. Deltas are attributed on the **next** `recordTick` (an action's outcome is only visible one tick later); before that fix, synchronous eval/marble runs compared same-tick states and reported "no effect" for every action of the run
...
>>>>>>> 0caa40d5 (fix eval-mode play quality: reward attribution, prompt contradictions, action caps)
...
When explicitly enabled, the **marble run** (`lib/attract-coordinator.js`, a singleton state machine) plays a finite portion of the eval playlist on the single Java process and broadcasts it live. It reuses the eval harness: each case runs through `runEvalCase` (`batch-evaluator.js`) with a **broadcast tee** — `createEventSink(broadcastIo)` forwards the buffered sink events to the real Socket.IO server, and `onCaseStart` hands the coordinator a live `{processId, llmClient}` handle. A walk-up has priority: `server.js` `/api/game/start` calls `coordinator.beginWalkup()` (disconnect the live case — which resolves the `run-summary` the eval was awaiting, so it unwinds through its own `finally` — then await `game-manager.stopGameAndWait()` to free port 8080), plays, then `endWalkup()` resumes the run within its case and wall-clock budgets. Starting requires `MARBLE_RUN_ENABLED=true` and operator authentication. Boot-time start also requires `MARBLE_RUN_AUTOSTART=true`. Control surface: `routes/marble.js` (`POST /api/marble/start|stop`, `GET /api/marble/state`). New events: `marble-run-state`, `case-started`, `case-completed`.
...
| `telemetry-store.js` | Event capture (`track`), batched async flush to Supabase (idempotent on `event_id`, retried on transient failures), local `data/telemetry-events.jsonl` fallback with **automatic drain** back into Supabase on recovery (`drainFallback`/`pump`), dashboard read models (`getDashboardSnapshot`) |
| `usage-guardrail.js` | Shared Ollama Cloud and OpenRouter call caps (hourly/daily/monthly/session, env-tunable, persisted buckets); enforced before each remote provider call — see LLM Routing |
...
- `.env` (repo root, not committed) — `OLLAMA_API_KEY` (primary) + `OPENROUTER_API_KEY` (fallback). Telemetry is also configured here (written by `setup-supabase-telemetry.sh`): `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_DB_PASSWORD` (for psql/migrations), `SUPABASE_TELEMETRY_TABLE`, plus `TELEMETRY_ENABLED` / `TELEMETRY_BATCH_SIZE` / `TELEMETRY_FLUSH_MS` / `TELEMETRY_FALLBACK_MODE` / `TELEMETRY_STORE_PROMPTS`. See `.env.example` for the full list. The `STRATEGY_MEMORY_DIR` env var can relocate the strategy-memory store; `MODEL_GUARDRAIL_HOURLY/DAILY/MONTHLY/SESSION/DISABLED/STATE` tune the remote-provider usage guardrail (see LLM Routing).
...
- **Remote-provider usage guardrail** (`lib/usage-guardrail.js`): Ollama Cloud and OpenRouter calls share hourly, daily, monthly, and per-session limits, configured with `MODEL_GUARDRAIL_HOURLY/DAILY/MONTHLY/SESSION`. A tripped guardrail blocks the call before the network request, skips every paid fallback, emits `llm-error`, and tracks a `system`/`guardrail_block` telemetry event. Railway deployments persist the buckets on the mounted volume and fail closed when storage is unreadable or unwritable; local runs use `web/data/usage-guardrail.json`.
- Provider → endpoint + auth: `ollama-cloud` → `config.ollamaCloud.apiUrl` + `OLLAMA_API_KEY`; `openrouter` → `config.openrouter.apiUrl` + `OPENROUTER_API_KEY`; `ollama-local` → `config.ollama.apiUrl` (no key); `legion-vllm` → `config.legion.apiUrl` + optional `LEGION_API_KEY` bearer. `LEGION_VLLM_URL` overrides the legion endpoint, but it is applied by `runtime-config.js` at config load — for the web server that means it must be in the **process environment** at startup (systemd `Environment=`), not only in root `.env`, because `getConfig()` caches at require time before `startServer()` loads `.env`. Eval scripts (`run-arcade-eval.js`) load `.env` before their first `getConfig()`, so `.env` works there. The Legion box points legion-vllm at its systemd vLLM (`localhost:1234`).
- A registry entry in `data/finetune-models.json` may set `"finetuned": false` to register a plain machine-local model (e.g. a base model on a local vLLM endpoint) — it merges into the catalog with its declared provider (`legion-vllm`, `ollama-local`) without wearing the fine-tuned badge. The file stays gitignored, so these entries are per-machine.
- **Model availability** (`isModelAvailable`, `availableEvalModels` in `models.js`): a model is available when its provider has a key (ollama-cloud also counts an OpenRouter key if the entry has a `fallback` slug) or is a machine-local provider. `/api/models` annotates each entry with `available`; the frontend picker defaults to the first *available* featured model (else first available, else first), and the attract coordinator passes `availabilityEnv: process.env` to `buildArcadeEvalPlan` so a keyless kiosk rotates its registry locals instead of dead cloud models. Without `availabilityEnv`, `buildArcadeEvalPlan` keeps its deterministic featured default (tests rely on this).
...
**Trace persistence (2026-07-26):** `play-trace-store.saveTrace` writes through to a Postgres `play_traces` table (`lib/trace-archive.js`, fire-and-forget) whenever `DATABASE_URL` is set — on Railway that means human plays survive redeploys instead of dying with the container filesystem. Schema ships as `postgres/migrations/005_create_play_traces.sql` (applied by the existing `npm run wall:migrate` pre-deploy step). A training box pulls the archive down into `data/play-traces/` with `DATABASE_URL=... node scripts/sync-traces.js` (idempotent by traceId via `importTrace`), after which `finetune:prepare` reads the remote plays exactly like local ones.
...
- **Paid background inference is disabled on Railway.** Keep `MARBLE_RUN_ENABLED=false`, `MARBLE_RUN_AUTOSTART=false`, `CADAVRE_MODEL_WARMER_ENABLED=false`, and `CADAVRE_READY_GENERATION_ENABLED=false`. Operator endpoints also require their feature flag and `MODEL_CONTROL_TOKEN`.
...
- `POST /api/game/start` takes the model id in a **`model`** field — passing `modelId` (the name used on socket/telemetry payloads) silently yields `this.model = undefined` and every Ollama call 404s with `model "" not found`. Model runs have action, provider-call, and wall-clock limits; the owning browser socket and `pagehide` handler stop abandoned runs.
