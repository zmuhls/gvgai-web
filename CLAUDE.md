...
- **Railway**: project/service `inference-arcade`, built from the root `Dockerfile` per `railway.json` (healthcheck `/api/games`). Deploy with `railway up --service inference-arcade --detach` from the repo root — GitHub push-to-deploy is **not** connected (the Railway GitHub app lacks access to `zmuhls/gvgai-web`), so pushing to master does not redeploy by itself.
