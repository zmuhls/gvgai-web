...
- Provider → endpoint + auth: `ollama-cloud` → `config.ollamaCloud.apiUrl` + `OLLAMA_API_KEY`; `openrouter` → `config.openrouter.apiUrl` + `OPENROUTER_API_KEY`; `ollama-local` → `config.ollama.apiUrl` (no key). `OLLAMA_LOCAL_API_URL` overrides the ollama-local **chat** endpoint per machine (read at `LLMClient` construction, like the API keys, because root `.env` loads after `getConfig()` caches); `ollama-loader.js` still derives the real Ollama daemon origin from `config.ollama`, so GGUF loading does not follow the override. The Legion box uses this to point ollama-local at its systemd vLLM (`localhost:1234`).
- A registry entry in `data/finetune-models.json` may set `"finetuned": false` to register a plain machine-local model (e.g. a base model on a local vLLM endpoint) — it merges into the catalog and routes to `ollama-local` without wearing the fine-tuned badge. The file stays gitignored, so these entries are per-machine.
