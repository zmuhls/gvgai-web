...
The web frontend spawns the Java process automatically via `game-manager.js` when a game is started from the browser.
...
- `.env` (repo root, not committed) — `OLLAMA_API_KEY` (primary) + `OPENROUTER_API_KEY` (fallback)
