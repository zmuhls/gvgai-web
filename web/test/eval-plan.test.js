...
test('arcade eval plan covers the featured models across three arcade games', () => {
  ...
  assert.deepEqual(plan.gameIds, [50, 26, 15]);
  assert.equal(plan.games[0].name, 'hungrybirds');
  assert.equal(plan.games[1].name, 'crossfire');
  assert.equal(plan.games[2].name, 'camelRace');
...
test('arcade eval plan can include more games without changing model coverage', () => {
  ...
  assert.deepEqual(plan.modelIds, FEATURED_MODEL_IDS);
  assert.deepEqual(plan.gameIds, [50, 26, 15, 13, 20]);
  assert.equal(plan.cases.length, plan.modelIds.length * plan.gameIds.length * plan.strategies.length);
});
...
test('arcade eval plan can cover the full model-native starter set', () => {
  ...
  assert.equal(plan.games[9].name, 'aliens');
  assert.equal(plan.cases.length, plan.modelIds.length * plan.gameIds.length * plan.strategies.length);
});
...
test('eval case filtering targets games, models, strategies, and caps run count', () => {
  ...
  assert.ok(cases.every(evalCase => [50, 26].includes(evalCase.gameId)));
